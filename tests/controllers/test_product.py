...
async def test_controller_get_should_return_not_found(client, products_url):
    response = await client.get(f"{products_url}4fd7c035-a3a0-4c1f-a78d-d24aa81e7dca")

    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert response.json() == "Product not found whith filter: 4fd7c035-a3a0-4c1f-a78d-d24aa81e7dca"


@pytest.mark.usefixtures("products_inserted")
async def test_controller_query_should_return_success(client, products_url, products_inserted):
    response = await client.get(products_inserted)
...
async def test_controller_delete_should_return_not_found(client, products_url):
    response = await client.delete(f"{products_url}4fd7c035-a3a0-4c1f-a78d-d24aa81e7dca")
